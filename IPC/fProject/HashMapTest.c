...
UNIT(hashBuild)
	...
	h = HashMap_Create(6,1,mod,Person_Eq2);
	ASSERT_THAT(h != NULL);
	HashMap_Destroy(&h,NULL,NULL);
END_UNIT
UNIT(hashBuild_capacityZero)
	HashMap *h;
	h = HashMap_Create(0,1,mod,Person_Eq2);
	ASSERT_THAT(NULL == h);
END_UNIT

UNIT(hashBuild_Null_param)
	HashMap *h;
	h = HashMap_Create(53,1,NULL,Person_Eq2);
	ASSERT_THAT(NULL == h);
	h = HashMap_Create(53,1,mod,NULL);
	ASSERT_THAT(NULL == h);
END_UNIT

UNIT(HashSetInsert_normal)
	HashMap *h;
	h = HashMap_Create(SIZE,1,mod,Person_Eq2);
	ASSERT_THAT(NULL != h);
