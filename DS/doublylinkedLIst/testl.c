...
UNIT(List_t_is_empty_normal)
    List_t *q = ListCreate();
    ASSERT_THAT( ListIsEmpty(q) != 0);
    ListDestroy(q);
END_UNIT

UNIT(List_is_empty_after_remove)
    int item = 55;
    List_t *q = ListCreate();
    ASSERT_THAT( ListPushHead(q, item) == ERR_OK );
    ASSERT_THAT( ListPopTail(q, &item) == ERR_OK );
    ASSERT_THAT( ListIsEmpty(q) == 0);
    ListDestroy(q);
END_UNIT
...
UNIT(List_t_full_remove_one_add_one)
    ...
    List_t *q = ListCreate();
    ASSERT_THAT( ListPushHead(q, 45) == ERR_OK );
    ASSERT_THAT( ListPushTail(q, 23) == ERR_OK );
    ASSERT_THAT( ListPopHead(q, &item) == ERR_OK);
    ASSERT_THAT( ListPushHead(q, 2) == ERR_OK );
    ASSERT_THAT( ListPopTail(q, &item) == ERR_OK); 
    ASSERT_THAT( ListPopTail(q, &item) == ERR_OK); 
    ListDestroy(q);
END_UNIT
...
UNIT(List_t_print_after_insert_and_remove)
    int item;
    List_t *q = ListCreate();
    ASSERT_THAT( ListPushTail(q, 1) == ERR_OK );
    ASSERT_THAT( ListPushHead(q, 9) == ERR_OK );

    ASSERT_THAT( ListPopHead(q, &item) == ERR_OK );
    ASSERT_THAT( ListPushTail(q, 2) == ERR_OK );
    ASSERT_THAT( ListPopTail(q, &item) == ERR_OK );
    ASSERT_THAT( ListPopHead(q, &item) == ERR_OK );
    ASSERT_THAT( ListPushHead(q, 78) == ERR_OK );
    ASSERT_THAT( ListPushTail(q, 23) == ERR_OK );
